...
BOOL CNetRcv::ConnectToCmdServer()
{
    ...
    if(!m_sock.ReadSockString(m_connect.cfd,buf,sizeof(buf),rn))
        return 0;
    sscanf(buf,"%d",&cmd);
    if(cmd!=100)
        return 0;
    ...
    if(!m_sock.ReadSockResponse(m_connect.cfd,buf,sizeof(buf),rn,102))
        return 0;
    
    sprintf(buf,"%s\r\n",m_connect.passwd);
    if(!m_sock.write_all(m_connect.cfd,buf,strlen(buf)))
        return 0;
    if(!m_sock.ReadSockResponse(m_connect.cfd,buf,sizeof(buf),rn,107)){
    ...
    }else if(!m_sock.ReadSockResponse(m_connect.cfd,buf,sizeof(buf),rn,108)){
        //		m_logfile.WriteTolog(buf,"err");
        return 0;
    }
...
{
    char err[]="Can not connect to data server";
    char err1[]="err";
    char rn[]="\r\n";
    ...
    if(m_connect.dfd<0){
        m_logfile.WriteTolog(err,err1);
        m_connect.dfd=-1;
        return 0;
    }
    //printf("read 104\n");
    if(!m_sock.ReadSockResponse(m_connect.cfd,tmp,sizeof(tmp),rn,104))
    {
        m_logfile.WriteTolog(tmp,err1);
        return 0;
    }
...
BOOL CNetRcv::AskData()
{
    ...
    if(!m_b109){
        if(!m_sock.ReadSockResponse(m_connect.cfd,buf,sizeof(buf),rn,109))
            return 0;
    }
    ...
    if(!m_sock.ReadSockResponse(m_connect.cfd,buf,sizeof(buf),rn,110))
        return 0;
    ...
    if(!m_sock.ReadSockResponse(m_connect.cfd,buf,sizeof(buf),rn,130))
        return 0;
    ...
    if(!m_sock.ReadSockResponse(m_connect.cfd,buf,sizeof(buf),rn,133))
        return 0;
...
{char dataInterrupt[]="Data interrupt!";
    long ctime;
    char err1[]="err";
    ...
    if(ctime-m_elapse >= 60){
        m_logfile.WriteTolog(dataInterrupt,err1);
