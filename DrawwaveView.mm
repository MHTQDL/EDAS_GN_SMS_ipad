...
//	k= m_dspctl.scantime*120/m_rectangle.size.width;
	...
	while(pchan!=NULL){
		...
		else sprintf(ss,"%ld %s",(long)pchan->dsp.max,unit[pchan->dsp.unit]);
		
        labels = [[NSString alloc ] initWithCString:ss encoding:NSUTF8StringEncoding]; //initWithCString: ss ];
		...
		*/
		if(pchan->dsp.unit!=0)
			sprintf(ss,"%.3G %s",pchan->dsp.min,unit[pchan->dsp.unit]);
		else sprintf(ss,"%ld %s",(long)pchan->dsp.min,unit[pchan->dsp.unit]);
...
-(void) CreateStnButton  :(int)l : (int)y : (NSString *) title{ 
	...
	if(m_stn_btn[l]==nil)
	{
		...
		m_stn_btn[l].titleLabel.lineBreakMode   =  NSLineBreakByTruncatingTail;//UILineBreakModeTailTruncation;
...
-(void) OnScantime{
	...
	UIActionSheet * menu=[[UIActionSheet alloc]
                          initWithTitle:@"选择显示时间"
                          delegate:self cancelButtonTitle:@"取消"
                          destructiveButtonTitle: nil
                          otherButtonTitles: @"10秒",@"30秒", @"60秒", @"120秒", @"180秒", @"300秒",@"600秒",nil];
...
- (void)dealloc {
	...
	{
		if(m_stn_btn[i]!=nil)
			[m_stn_btn[i] release];//改动 [m_stn_btn release];
	}
    [super dealloc];
}
-(void) CreateStatusbar{
